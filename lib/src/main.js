...
function saveFile(filename, data, overwrite) {
  return new Promise((resolve, reject) => {
    overwrite
      ...
      : fsu.writeFileUnique(filename.replace('.html', '{_###}.html'), data, { force: true },
...
/**
 * Construct the path/name of the HTML file to be saved
 *
 ...
 *
 * @return {string} Fully resolved path
 */
function getHtmlFilename({ reportDir, reportFilename, timestamp }) {
  let ts = '';
  if (timestamp) {
    ts = `_${new Date().getTime()}`;
  }
  const filename = `${reportFilename.replace(fileExtRegex, '')}${ts}.html`;
...
function getOptions(opts) {
  ...
  mergedOptions.reportHtmlFile = getHtmlFilename(mergedOptions);
...
function create(data, opts) {
  ...
  const { timestamp, overwrite } = options;
  return saveFile(reportFilename, html, timestamp ? false : overwrite)
    .then(savedFile => (options.autoOpen && openFile(savedFile)) || savedFile);
}
